...
**Watch workers:** press **Ctrl+Shift+W** while they run, or use **`/workers`**. Left/Right or Tab cycles between the overview and each child's messages and tool calls/results; Up/Down and PageUp/PageDown scroll, End follows new output, and Escape returns to the main conversation. Workers continue running while the viewer is open. This first version is an inspection view, not an input box for messaging children. Saved worker views are available after resuming their parent session. Large individual messages are previewed up to 30,000 characters; the displayed transcript path contains the complete record.

The main delegation card also shows each worker's current tool, target file/command, status and elapsed time. **Ctrl+O** expands recent activity and response excerpts. This UI activity stays out of the orchestrator's final tool-result text; it still receives concise worker results.
