...
**Experiment with models:** select a role with **Ctrl+Shift+A** or `/agent`, then choose a model with **Ctrl+L** (or cycle models with Ctrl+P). **Ctrl+T** cycles reasoning and **Shift+Tab** cycles agents; see [PiAstra editor shortcuts](docs/shortcuts.md) for the full leader-key map. PiAstra remembers these choices per role in the current session, restores them on resume, and uses them for future delegated workers of that role. Running workers keep their launch settings. `/piastra` shows the current choices.
