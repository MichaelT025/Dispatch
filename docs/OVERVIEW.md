...
- `Esc` stops the orchestrator's turn only. To stop everything, press `Esc` again within 1.5 s or use `Ctrl+X` then `c`. Individual workers: `/cancel <id>` (`/cancel all`, or bare `/cancel` to pick), `x` twice in `/workers`, the Web UI's cancel button, or ask the orchestrator (`cancel_worker`). Each worker task has a 20-minute timeout. Timeout, cancellation, and failure results include an observed stopping point (last activity, in-flight tools, recent tool results, and partial response). All results list files observed in successful `edit`/`write` calls and a scoped shared-worktree `git diff --stat` against HEAD; untracked files are listed separately. Shell edits are not tracked, and the stat can include pre-existing or other workers' edits. Model errors are returned as-is, never silently swapped to another model.
