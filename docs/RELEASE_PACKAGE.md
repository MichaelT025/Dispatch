...
- Its manifest exposes only `dispatch`. Pi (see `PI_PINNED_VERSION` in `lib/pi-install.mjs`) is a pinned normal dependency, shared with the WebUI. Production dependencies combine the extension runtime and WebUI dependencies; legacy upstream WebUI/Tau/agegr trials and development tooling are excluded.
