...
await writeFile(path.join(cwd, 'fixture.txt'), 'before\n');
...
const cli = process.argv[2] || path.join(process.env.APPDATA, 'npm/node_modules/@earendil-works/pi-coding-agent/dist/bundle/cli.js');
const prompt = `Integration test. Use delegate for these three sequential batches, no other tools yourself:
...
console.log(JSON.stringify({ cwd, code, workers: workerResults.map(r => ({ role: r.role, model: r.model, ok: r.ok, text: r.text, transcript: r.transcript })), stderr }, null, 2));
if (code !== 0 || workerResults.length !== 4 || workerResults.some(r => !r.ok) || (await readFile(path.join(cwd, 'fixture.txt'), 'utf8')) !== 'after\n') process.exitCode = 1;
