...
/**
 * PiAstra vendored fork: disable the persistent above-editor TodoOverlay
 * widget. Only the visual overlay is switched off — the `todo` tool, the
 * `/todos` command, state/replay/session-isolation, and every lifecycle
 * handler remain upstream-verbatim. Exactly three guarded seams below:
 *
 *   1. `updateTodoOverlay` — returns immediately, so no TodoOverlay is ever
 *      constructed and no `rpiv-todos` widget is registered or removed.
 *   2. The collapse/expand shortcut registration is skipped.
 *   3. The overlay pre-warm timer is skipped.
 *
 * See FORK.md for the fork record.
 */
const TODO_OVERLAY_DISABLED = true;
...
export default function (pi: ExtensionAPI, importOverlay: TodoOverlayImporter = () => import("./todo-overlay.js")) {
	...
	async function updateTodoOverlay(
		...
		// PiAstra fork: overlay widget disabled — never load/construct/register it.
		if (TODO_OVERLAY_DISABLED) return;
	...
	// PiAstra fork: the collapse shortcut only exists to toggle the overlay
	// widget; it is not registered while the overlay is disabled.
	if (!TODO_OVERLAY_DISABLED && collapseKey !== COLLAPSE_KEY_OFF) {
	...
	// PiAstra fork: the pre-warm only serves the disabled overlay widget, so the
	// timer is not scheduled at all.
	if (!TODO_OVERLAY_DISABLED) {
