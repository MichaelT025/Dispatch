...
export interface CommandCodeRuntimeOptions<TProviderConfig> {
  ...
  /** Reapply account-specific catalog projection even if discovery used cache. */
  reregisterCachedModels?: boolean
  /** Account-specific catalogs must finish verification before CLI discovery exits. */
  awaitInitialRefresh?: boolean
...
export class CommandCodeRuntime<TProviderConfig, TContext extends CommandCodeCommandContext> {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.options.awaitInitialRefresh) await this.refresh()
  ...
  private async refreshCatalog(): Promise<CommandCodeRefreshResult> {
    ...
    try {
      ...
      const shouldRegister =
        ...
        loaded.source === "live" ||
        (this.options.reregisterCachedModels && loaded.models.length > 0) ||
  ...
  private registerCommands(): void {
    ...
    this.pi.registerCommand("commandcode-status", {
      ...
      handler: async (_args, ctx) => {
        ...
        const description = this.options.describeCatalog?.()
        ctx.ui.notify([formatCommandCodeStatus(status), description].filter(Boolean).join("\n"), status.warning ? "warning" : "info")
      },
    })
  }
}
