...
export const helpSections = [
  ...
  },
  {
    ...
    lines: [
      ...
      '- Delegation is asynchronous: delegate returns immediately and each',
      '  result comes back as a [dispatch-worker-result] message that wakes',
      '  the orchestrator. await_workers blocks for specific results,',
      '  cancel_worker stops one, continue_worker re-prompts a finished',
      '  worker in its own session (until the session is resumed).',
      ...
      '',
      'Cancelling workers:',
      '- Esc stops the orchestrator turn only. Esc again within 1.5s while',
      '  workers run cancels all of them; Ctrl+X then c stops everything at',
      '  once. /cancel <id>, /cancel all, or bare /cancel to pick one; in',
      '  /workers, press x twice on a worker.',
      '',
      'Watching workers:',
  ...
  },
  {
    ...
    lines: [
      ...
      '- /name <title> (Pi-provided): set the session display name manually.',
      '- /rename (Dispatch): regenerate the title with the fast model while idle.',
      '  Replaces an existing title; also works when autoTitle is false.',
  ...
  },
  {
    ...
    lines: [
      ...
      '- Esc twice (workers running): cancel all workers.',
  ...
  },
  {
    ...
    lines: [
      ...
      '- "N workers still running...": wait for their results or /cancel',
      '  them before switching sessions. Same for compaction/branch-summary',
      '  waits.',
