...
const canonicalPath = path => realpathSync.native(path);
const temp = prefix => {
...
test('runUpdate requires prefix writable, not just package dir', async () => {
  ...
  const accessImpl = async p => {
    if (canonicalPath(p) === canonicalPath(prefix)) throw new Error('EACCES');
  };
  await assert.rejects(() => runUpdate({
    paths, env: {}, output: o.stream,
    lookup: async () => ({ currentVersion: '0.1.0', version: '0.2.0' }),
...
test('runUpdate verifies exact args and version; failure paths throw', async () => {
  ...
  assert.equal(rc, 0);
  assert.deepEqual(seen, [['install', '--global=false', '--prefix', canonicalPath(prefix), '--save-optional', `${PACKAGE_NAME}@0.2.0`]]);
...
test('runUpdate global uses -g without --global=false', async () => {
  ...
  assert.equal(rc, 0);
  assert.deepEqual(seen, [['install', '-g', '--prefix', canonicalPath(prefix), `${PACKAGE_NAME}@0.2.0`]]);
});
