...
test('bundled Command Code provider loads through Pi offline', async () => {
  ...
  const previousClassification = process.env.COMMANDCODE_MODEL_CLASSIFICATION;
  ...
  try {
    ...
    if (previousClassification === undefined) delete process.env.COMMANDCODE_MODEL_CLASSIFICATION;
    else process.env.COMMANDCODE_MODEL_CLASSIFICATION = previousClassification;
