...
test('managedExtensionPaths returns seven entries in order', () => {
  const root = '/pkg';
...
test('first setup seeds seven extensions, defaults, web policy; no auth touched', async () => {
  ...
  assert.equal(state.managedExtensionPaths.length, 7);
...
test('seed preserves custom settings and nested web settings', async () => {
  ...
  assert.equal(settings.extensions.length, 8);
  const client = await readJson(join(paths.webDir, 'client-state.json'));
