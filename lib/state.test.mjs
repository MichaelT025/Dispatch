...
test('managedExtensionPaths returns nine entries in order', () => {
  const root = '/pkg';
  assert.deepEqual(managedExtensionPaths(root), [
    ...
    join(root, 'extensions/pi-usage/index.ts'),
  ]);
});
...
test('first setup seeds nine extensions, defaults, web policy; no auth touched', async () => {
  ...
  assert.equal(state.managedExtensionPaths.length, 9);
...
test('seed preserves custom settings and nested web settings', async () => {
  ...
  assert.equal(settings.extensions.length, 10);
  const client = await readJson(join(paths.webDir, 'client-state.json'));
