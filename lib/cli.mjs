...
import { checkForUpdate, runUpdate } from './updates.mjs';
import { setUpdateNoticePromise } from './update-notice.mjs';
import { trackDispatchInstance } from './instances.mjs';
...
// Pi consumes the next token for these options even if it starts with '-'.
...
export function shouldCheckForUpdate(args, { stdinTTY = process.stdin.isTTY, stdoutTTY = process.stdout.isTTY } = {}) {
  if (!stdinTTY || !stdoutTTY) return false;
  const flags = topLevelArgs(args);
  if (flags.some(arg => ['--mode', '--print', '-p', '--export', '--list-models'].includes(arg))) return false;
  return !['auth', 'install', 'remove', 'uninstall', 'list', 'config'].includes(args[0]);
}
...
export function resolvePiCli() {
  ...
  if (manifest.version !== PI_PINNED_VERSION || typeof manifest.bin?.pi !== 'string') {
    throw new Error(`Dispatch requires its pinned Pi ${PI_PINNED_VERSION} runtime. Reinstall the Dispatch package.`);
  }
...
async function startPi({ paths, args, env }) {
  ...
  setUpdateNoticePromise(shouldCheckForUpdate(args) ? checkForUpdate({ currentVersion: env.DISPATCH_VERSION, env }) : Promise.resolve());
  const release = await trackDispatchInstance(paths, 'cli');
  ...
  try { await import(entry.href); } catch (error) { release(); throw error; }
}
...
async function startWeb({ paths, options, env, output }) {
  ...
  applyEnvironment(env);
  setUpdateNoticePromise(checkForUpdate({ currentVersion: env.DISPATCH_VERSION, env }));
  await trackDispatchInstance(paths, 'web');
...
export async function runDispatch(argv, {
  ...
  if (options.mode === 'update') {
    ...
    process.once('SIGINT', onInterrupt);
    process.once('SIGTERM', onInterrupt);
    try { return await update({ paths, env, output, errorOutput, signal: controller.signal }); }
    finally {
      process.removeListener('SIGINT', onInterrupt);
      process.removeListener('SIGTERM', onInterrupt);
    }
