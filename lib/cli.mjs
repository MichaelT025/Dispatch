...
async function startPi({ paths, args, env }) {
  ...
  const interactive = !flags.some(arg => ['--mode', '--print', '-p', '--export', '--list-models'].includes(arg)) &&
    !['install', 'remove', 'uninstall', 'list', 'config'].includes(args[0]);
  setUpdateNoticePromise(interactive ? checkForUpdate({ currentVersion: env.DISPATCH_VERSION, env }) : Promise.resolve());
