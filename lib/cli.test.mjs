...
test('startup update checks require an interactive CLI and exclude auth/print modes', () => {
  assert.equal(shouldCheckForUpdate([], { stdinTTY: true, stdoutTTY: true }), true);
  assert.equal(shouldCheckForUpdate(['auth', 'check'], { stdinTTY: true, stdoutTTY: true }), false);
  assert.equal(shouldCheckForUpdate(['--mode', 'rpc'], { stdinTTY: true, stdoutTTY: true }), false);
  assert.equal(shouldCheckForUpdate(['--print'], { stdinTTY: true, stdoutTTY: true }), false);
  assert.equal(shouldCheckForUpdate([], { stdinTTY: false, stdoutTTY: true }), false);
  assert.equal(shouldCheckForUpdate([], { stdinTTY: true, stdoutTTY: false }), false);
});
